...
青龙里执行 `playids` 之前，必须先保证账号处于已登录状态。最常用的是 Cookie 登录：

```shell
export NCMCTL_QINGLONG_LOGIN_MODE=cookie
export NCMCTL_QINGLONG_LOGIN_COOKIE='MUSIC_U=xxx; __csrf=yyy; NMTID=zzz;'
```
...
export NCMCTL_QINGLONG_LOGIN_ACCOUNT=<你的UUID>
export NCMCTL_QINGLONG_LOGIN_PASSWORD=<你的密码>
export NCMCTL_QINGLONG_LOGIN_COOKIECLOUD_SERVER=http://127.0.0.1:8088
```

先运行一次 `ncmctl登录` 任务，确认登录成功后，再开启 `playids`。

### 单账号 Cookie 约束

`NCMCTL_QINGLONG_LOGIN_COOKIE` 当前只支持单账号，不支持在一个环境变量或一个 cookie 文件里维护多个账号。
...
# 或导入cookie文件内容
export NCMCTL_QINGLONG_LOGIN_COOKIE=./cookie.txt
```

补充说明：

- 使用字符串模式时，建议整体使用单引号包裹，避免 `;` 被 shell 拆分
- cookie 内容至少需要包含 `MUSIC_U`，否则登录会失败

### 青龙对 `playids` 的支持

`playids` 不会默认执行，需要显式开启。它和 `scrobble` 不同：

- 使用指定 `songId` 池播放
- 按真实完整时长播放后再上报
- 即使账号已满级也会执行
- 仍然受每日 300 首总上限约束，并与 `scrobble` 共用计数

推荐执行顺序：

1. 先完成登录
2. 准备歌曲 ID 列表
3. 设置 `NCMCTL_QINGLONG_PLAYIDS=true`
4. 再运行青龙一键任务

相关环境变量：

- `NCMCTL_QINGLONG_PLAYIDS` 是否开启指定歌曲完整播放，默认 `false`
- `NCMCTL_QINGLONG_PLAYIDS_IDS` 指定歌曲 ID 列表，逗号分隔
- `NCMCTL_QINGLONG_PLAYIDS_IDS_FILE` 指定歌曲 ID 文件路径
- `NCMCTL_QINGLONG_PLAYIDS_NUM` 指定歌曲完整播放数量，默认 300
- `NCMCTL_QINGLONG_PLAYIDS_GAP_MIN` 两首歌之间最小随机间隔秒数，默认 5
- `NCMCTL_QINGLONG_PLAYIDS_GAP_MAX` 两首歌之间最大随机间隔秒数，默认 20

环境变量规则：

- `NCMCTL_QINGLONG_PLAYIDS` 不设置或设为 `false` 时，不会执行 `playids`
- `NCMCTL_QINGLONG_PLAYIDS_IDS` 和 `NCMCTL_QINGLONG_PLAYIDS_IDS_FILE` 可以同时设置
- 同时设置时，脚本会两者合并后去重
- 两者都不设置但开启了 `NCMCTL_QINGLONG_PLAYIDS=true` 时，脚本会直接报错退出
- `NCMCTL_QINGLONG_PLAYIDS_NUM` 未设置时等价于 `300`
- `NCMCTL_QINGLONG_PLAYIDS_GAP_MIN` / `MAX` 未设置时分别等价于 `5` / `20`

示例一：直接通过环境变量传入歌曲 ID 列表

```shell
export NCMCTL_QINGLONG_PLAYIDS=true
export NCMCTL_QINGLONG_PLAYIDS_IDS=2600804126,1984580503,3370932775
export NCMCTL_QINGLONG_PLAYIDS_NUM=10
export NCMCTL_QINGLONG_PLAYIDS_GAP_MIN=0
export NCMCTL_QINGLONG_PLAYIDS_GAP_MAX=0
```

示例二：通过文件传入歌曲 ID 列表

```shell
cat >/ql/data/song_ids.txt <<'EOF'
2600804126
1984580503
3370932775
EOF
...
export NCMCTL_QINGLONG_PLAYIDS=true
export NCMCTL_QINGLONG_PLAYIDS_IDS_FILE=/ql/data/song_ids.txt
export NCMCTL_QINGLONG_PLAYIDS_NUM=50
```

如果同时设置 `NCMCTL_QINGLONG_PLAYIDS_IDS` 和 `NCMCTL_QINGLONG_PLAYIDS_IDS_FILE`，脚本会两者合并后去重，行为和 `ncmctl playids --ids --ids-file` 一致。
...
- 青龙一键任务会先跑原有开启的任务
- 当 `NCMCTL_QINGLONG_PLAYIDS=true` 时，会额外执行 `ncmctl playids ...`
- 日志里会显示当前账号、歌曲池、正在播放第几首、本首结果、最终汇总

常见排查：

- 报 `need login`：说明当前 cookie 无效或未登录
- 报 `song ids is empty`：说明歌曲池为空或文件内容无效
- 报 `today play limit 300 completed`：说明当日总量已到上限
- 开始后长时间无下一条结果：通常是在等待整首歌完整播放结束，不是卡死
...
环境变量配置：
可参考本文档顶部“本次新增功能说明”中的 Cookie 配置示例。
